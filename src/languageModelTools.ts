...
class GetDiagnosticsTool implements vscode.LanguageModelTool<GetDiagnosticsInput> {
  async invoke(
    ...
    try {
      const input = options.input;
      console.log(
        `[GetDiagnosticsTool] Getting diagnostics for: ${input.file}`,
      );
...
class GetDocumentationLinkTool implements vscode.LanguageModelTool<GetDocumentationLinkInput> {
  ...
  async invoke(
    ...
    try {
      const input = options.input;
      console.log(
        `[GetDocumentationLinkTool] Searching docs for: ${input.functionName}`,
      );
  ...
  private async loadDocsData(): Promise<void> {
    try {
      ...
      console.log(
        `[GetDocumentationLinkTool] Loaded ${this.docsData?.length || 0} documentation entries`,
      );
    } catch (error: any) {
      console.error(
        `[GetDocumentationLinkTool] Failed to load docs: ${error.message}`,
      );
      this.docsData = null;
    }
  }

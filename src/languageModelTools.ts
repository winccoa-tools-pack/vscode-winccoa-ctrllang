...
export class LanguageModelToolsService {
    ...
    register(context: vscode.ExtensionContext): void {
        ...
        this.disposables.push(
            vscode.lm.registerTool('ctl_syntax_check', new SyntaxCheckTool())
        );
...
class GetSymbolInfoTool implements vscode.LanguageModelTool<GetSymbolInfoInput> {
    async invoke(
        ...
        try {
            const input = options.input;
            console.log(`[GetSymbolInfoTool] Getting symbol info at ${input.file}:${input.line}:${input.column}`);
...
class FindReferencesTool implements vscode.LanguageModelTool<FindReferencesInput> {
    async invoke(
        ...
        try {
            const input = options.input;
            console.log(`[FindReferencesTool] Finding references at ${input.file}:${input.line}:${input.column}`);
...
class GotoDefinitionTool implements vscode.LanguageModelTool<GotoDefinitionInput> {
    async invoke(
        ...
        try {
            const input = options.input;
            console.log(`[GotoDefinitionTool] Finding definition at ${input.file}:${input.line}:${input.column}`);

  ...
  public async formatDocument(document: vscode.TextDocument): Promise<boolean> {
    ...
    if (!config.get<boolean>("enabled", true)) {
      ExtensionOutputChannel.trace(
        "Formatter",
        "Astyle formatter disabled in settings",
      );
      return false;
    }
    ...
    ExtensionOutputChannel.debug(
      "Formatter",
      `Formatting file: ${path.basename(filePath)}`,
    );
    ...
    try {
      ...
      ExtensionOutputChannel.error(
        "Formatter",
        `Error formatting file: ${err.message}`,
        err,
      );
      return false;
    }
  ...
  private async resolveAstylePaths(): Promise<{
    ...
    if (customBinaryPath && customConfigPath) {
      ExtensionOutputChannel.info(
        "Formatter",
        "Using custom astyle paths from settings",
      );
      ...
      if (!fs.existsSync(customBinaryPath)) {
        ...
        vscode.window.showErrorMessage(
          `Astyle binary not found at: ${customBinaryPath}`,
        );
      ...
      if (!fs.existsSync(customConfigPath)) {
        ...
        vscode.window.showErrorMessage(
          `Astyle config not found at: ${customConfigPath}`,
        );
        return { astylePath: null, configPath: null };
      }
    ...
    ExtensionOutputChannel.debug(
      "Formatter",
      "Using WinCC OA installation paths (default)",
    );
    ...
    if (!projectPaths || !projectPaths.installPath) {
      ExtensionOutputChannel.warn(
        "Formatter",
        "WinCC OA installation path not found",
      );
      return { astylePath: null, configPath: null };
    }
    ...
    if (!fs.existsSync(astylePath)) {
      ExtensionOutputChannel.error(
        "Formatter",
        `Binary not found: ${astylePath}`,
      );
      vscode.window.showWarningMessage(
        `Astyle binary not found at: ${astylePath}`,
      );
      return { astylePath: null, configPath: null };
    }
    ...
    if (!fs.existsSync(configPath)) {
      ...
      vscode.window.showWarningMessage(
        `Astyle config not found at: ${configPath}`,
      );
      return { astylePath: null, configPath: null };
    }

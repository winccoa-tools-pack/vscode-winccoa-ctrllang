...
export class ProjectPathResolver {
    ...
    switch (mode) {
      ...
      default:
        ExtensionOutputChannel.warn(
          "PathResolver",
          `Unknown path source mode: ${mode}, falling back to workspace`,
        );
        return await this.getPathsFromWorkspace();
    }
  }
  ...
  private async getPathsFromWorkspace(): Promise<ProjectPaths | null> {
    ...
    ExtensionOutputChannel.info(
      "PathResolver",
      `Searching for WinCC OA project in ${workspaceFolders.length} workspace folder(s)...`,
    );
      ...
      try {
        if (fs.existsSync(configPath)) {
          ExtensionOutputChannel.success(
            "PathResolver",
            `✓ Found WinCC OA project at: ${folderPath}`,
          );
          ExtensionOutputChannel.info(
            "PathResolver",
          ...
          ExtensionOutputChannel.success(
            "PathResolver",
            "Project paths detected from workspace",
          );
          ExtensionOutputChannel.debug(
            "PathResolver",
            `  Main project: ${paths.projectPath}`,
          );
          ExtensionOutputChannel.debug(
            "PathResolver",
          ...
          ExtensionOutputChannel.debug(
            "PathResolver",
            `  Subprojects: ${paths.subProjects.length}`,
          );
          ExtensionOutputChannel.info(
            "PathResolver",
            `  Scripts paths: ${paths.scriptsPaths.length}`,
          );
          ExtensionOutputChannel.trace(
            "PathResolver",
            "Full paths object",
          ...
          ExtensionOutputChannel.debug(
            "PathResolver",
            `  ✗ No config/config found in ${folderPath}`,
          );
        }
      } catch (error) {
        ...
        ExtensionOutputChannel.error(
          "PathResolver",
          `Error checking folder ${folderPath}: ${err.message}`,
          err,
        );
      }
    }
    ...
    ExtensionOutputChannel.warn(
      "PathResolver",
      "No WinCC OA project (config/config) found in any workspace folder",
    );
    return null;
  }
  ...
  private getPathsFromConfig(): ProjectPaths | null {
    ...
    ExtensionOutputChannel.trace("PathResolver", "Manual configuration", {
      projectPath,
      installPath,
      subProjects,
      additionalScriptsPaths,
    ...
    ExtensionOutputChannel.success(
      "PathResolver",
      "Project paths loaded from configuration",
    );
    this.cachedPaths = paths;
    return paths;
  }
  ...
  private getPathsFromCore(): ProjectPaths | null {
    ...
    if (!coreExtension) {
      ExtensionOutputChannel.error(
        "PathResolver",
        "WinCC OA Core extension not found",
      );
    ...
    if (!coreExtension.isActive) {
      ...
      if (this.coreActivationAttempted) {
        ExtensionOutputChannel.warn(
          "PathResolver",
          "Core extension is not active yet",
        );
      } else {
        ExtensionOutputChannel.debug(
          "PathResolver",
          "Core extension not yet active (first call during startup)",
        );
    ...
    if (!currentProject) {
      ...
      ExtensionOutputChannel.debug(
        "PathResolver",
        "No project selected in Core extension, returning null for workspace fallback",
      );
      return null;
    }
    ...
    ExtensionOutputChannel.success(
      "PathResolver",
      "Project paths loaded from Core extension",
    );
    ExtensionOutputChannel.debug(
      "PathResolver",
   ...
   */
  private parseSubProjectsFromConfig(
    configPath: string,
    ...
    try {
      ...
      ExtensionOutputChannel.debug(
        "PathResolver",
        `Found ${subProjects.length} subprojects`,
      );
    } catch (err) {
  ...
  private detectInstallPath(configPath: string): string {
    try {
      ...
      for (const line of lines) {
        ...
        const match = trimmed.match(
          /^(?:pvss_path|winccoa_path|install_path)\s*=\s*["']([^"']+)["']/,
        );
        if (match) {
          ExtensionOutputChannel.debug(
            "PathResolver",
            `Detected install path: ${match[1]}`,
          );
          return match[1];
        }
      }
      ...
      ExtensionOutputChannel.trace(
        "PathResolver",
        `Could not detect install path: ${error.message}`,
      );
    }
    ...
    for (const defaultPath of defaultPaths) {
      if (fs.existsSync(defaultPath)) {
        ExtensionOutputChannel.debug(
          "PathResolver",
          `Using default install path: ${defaultPath}`,
        );
        return defaultPath;
      }
    }

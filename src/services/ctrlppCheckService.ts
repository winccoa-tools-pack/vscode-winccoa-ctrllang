...
export class CtrlppCheckService {
  ...
  constructor(outputChannel: vscode.OutputChannel) {
    this.diagnosticCollection =
      vscode.languages.createDiagnosticCollection("ctrlppcheck");
  ...
  public async checkFile(document: vscode.TextDocument): Promise<void> {
    ...
    if (!fs.existsSync(binaryPath)) {
      this.outputChannel.appendLine(
        `CtrlppCheck: Binary not found at: ${binaryPath}`,
      );
    ...
    try {
      const diagnostics = await this.runCtrlppCheck(
        binaryPath,
        filePath,
        config,
      );
      ...
      this.outputChannel.appendLine(
        `CtrlppCheck: Error checking file: ${error}`,
      );
    }
  }
  ...
  private async runCtrlppCheck(
    ...
    this.outputChannel.appendLine(
      `CtrlppCheck: Running: ${binaryPath} ${args.join(" ")}`,
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      process.on("close", (code) => {
        this.outputChannel.appendLine(
          `CtrlppCheck: Process exited with code ${code}`,
        );
        ...
        try {
          this.outputChannel.appendLine(
            `CtrlppCheck: XML output file: ${outputFile}`,
          );
          ...
          this.outputChannel.appendLine(
            `CtrlppCheck: Error parsing output: ${error}`,
          );
          reject(error);
        }
      });
  ...
  private parseXmlOutput(
    xmlFilePath: string,
    checkedFilePath: string,
  ): vscode.Diagnostic[] {
    if (!fs.existsSync(xmlFilePath)) {
      this.outputChannel.appendLine(
        `CtrlppCheck: XML output file not found: ${xmlFilePath}`,
      );
      return [];
    }
    ...
    try {
      ...
      this.outputChannel.appendLine(
        `CtrlppCheck: Found ${diagnostics.length} diagnostic(s)`,
      );
      return diagnostics;
    } catch (error) {

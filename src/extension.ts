...
export function activate(context: vscode.ExtensionContext) {
  ...
  ExtensionOutputChannel.info(
    "Extension",
    "  WinCC OA CTRL Language Support - Starting...",
  );
  ExtensionOutputChannel.info(
    "Extension",
    "═══════════════════════════════════════════════════════",
      ...
      for (const file of event.files) {
        if (file.fsPath.match(/\.(ctl|ctrl|ctlpp|ctrlpp)$/)) {
          ...
          if (document.getText().trim().length < 100) {
            ...
            const isTest =
              file.fsPath.includes("/tests/") || file.fsPath.includes("Test");
            const templateType = isTest ? "Test" : "Script";
            const templateFile = isTest
              ? "test-template.ctl"
              : "script-template.ctl";
  ...
  context.subscriptions.push(
    vscode.workspace.onDidSaveTextDocument(async (document) => {
      if (document.languageId !== "ctrl" && document.languageId !== "ctrlpp") {
        ExtensionOutputChannel.trace(
          "SaveHandler",
          `Ignoring non-ctrl file: ${document.fileName}`,
        );
        return;
      }
      ...
      const astyleConfig = vscode.workspace.getConfiguration(
        "winccoa.astyleFormatter",
      );
      ...
      if (astyleRunOnSave) {
        ExtensionOutputChannel.info(
          "Formatter",
          `Running Astyle formatter: ${path.basename(document.fileName)}`,
        );
        await astyleFormatterService.formatDocument(document);
      } else {
        ExtensionOutputChannel.trace(
          "Formatter",
          "Astyle formatter disabled (runOnSave=false)",
        );
      }

      // Then run WinCC OA Syntax Check (if enabled)
      const syntaxCheckConfig = vscode.workspace.getConfiguration(
        "winccoa.syntaxCheck",
      );
      const syntaxCheckOnSave = syntaxCheckConfig.get<boolean>(
        "executeOnSave",
        true,
      );
      ...
      if (syntaxCheckOnSave) {
        ExtensionOutputChannel.info(
          "SyntaxCheck",
          `Running WinCC OA Syntax Check: ${path.basename(document.fileName)}`,
        );
        await syntaxCheckService.checkFile(document);
      } else {
        ExtensionOutputChannel.trace(
          "SyntaxCheck",
          "WinCC OA Syntax Check disabled (executeOnSave=false)",
        );
      }
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration((e) => {
      if (e.affectsConfiguration("winccoa.ctrlLang")) {
        ExtensionOutputChannel.info(
          "Configuration",
          "Configuration changed, clearing project paths cache",
        );
        ExtensionOutputChannel.debug(
          "Configuration",
          `Affected sections: winccoa.ctrlLang`,
        );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("winccoa.formatDocument", async () => {
      ...
      if (
        editor.document.languageId !== "ctrl" &&
        editor.document.languageId !== "ctrlpp"
      ) {
        vscode.window.showErrorMessage(
          "Current file is not a Ctrl or Ctrl++ file",
        );
        return;
      }
      ...
      ExtensionOutputChannel.info(
        "Command",
        `Manual format: ${path.basename(editor.document.fileName)}`,
      );
      const success = await astyleFormatterService.formatDocument(
        editor.document,
      );
      if (success) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("winccoa.runSyntaxCheck", async () => {
      ...
      ExtensionOutputChannel.info(
        "Command",
        `Manual syntax check: ${path.basename(editor.document.fileName)}`,
      );
    ...
    vscode.commands.registerCommand("winccoa.openDocumentation", async () => {
      ...
      if (!wordRange) {
        vscode.window.showInformationMessage(
          "No function name found at cursor position",
        );
        return;
      }
      ...
      try {
        ExtensionOutputChannel.info(
          "Documentation",
          `Requesting documentation for: ${functionName}`,
        );
        const docUrl = await client.sendRequest(
        ...
        if (docUrl && typeof docUrl === "string") {
          ExtensionOutputChannel.success(
            "Documentation",
            `Opening documentation: ${docUrl}`,
          );
          vscode.env.openExternal(vscode.Uri.parse(docUrl));
        } else {
          ExtensionOutputChannel.warn(
            "Documentation",
            `No documentation found for '${functionName}'`,
          );
          vscode.window.showInformationMessage(
            `No documentation found for '${functionName}'`,
          );
        ...
        ExtensionOutputChannel.error(
          "Documentation",
          `Error getting documentation: ${err.message}`,
          err,
        );
...
export function deactivate(): Thenable<void> | undefined {
  ExtensionOutputChannel.info(
    "Extension",
    "WinCC OA CTRL Language Support is deactivating...",
  );
  if (!client) {
    return undefined;
  }
...
function startLanguageServer(context: vscode.ExtensionContext) {
  ...
  startPromise.then(() => {
    ...
    client.onRequest("custom/getProjectFromCore", async () => {
      ...
      const projectPaths =
        await ProjectPathResolver.getInstance().getProjectPaths();
...
async function performStartupDiagnostics(context: vscode.ExtensionContext) {
  ...
  try {
    ...
    if (projectPaths) {
      ...
      ExtensionOutputChannel.info(
        "Diagnostics",
        `   Install Path: ${projectPaths.installPath}`,
      );
      ExtensionOutputChannel.info(
        "Diagnostics",
        `   Subprojects: ${projectPaths.subProjects.length}`,
      );
      ExtensionOutputChannel.info(
        "Diagnostics",
        `   Scripts Paths: ${projectPaths.scriptsPaths.length}`,
      );
      ExtensionOutputChannel.trace(
        "Diagnostics",
        "   Full project paths",
      ...
      ExtensionOutputChannel.warn(
        "Diagnostics",
        "⚠️  No WinCC OA Project detected",
      );
      if (pathSource === "workspace") {
        ExtensionOutputChannel.info(
          "Diagnostics",
          "   → No config/config file found in workspace",
        );
        ExtensionOutputChannel.info(
          "Diagnostics",
          "   → Open a WinCC OA project folder or configure manually",
        );
        vscode.window
          .showWarningMessage(
        ...
        ExtensionOutputChannel.warn(
          "Diagnostics",
          "   → Manual mode: Check your path configuration",
        );
        ExtensionOutputChannel.info(
          "Diagnostics",
          "   → Verify winccoa.ctrlLang.projectPath setting",
        );
      }
    }
  } catch (error) {
    ...
    ExtensionOutputChannel.error(
      "Diagnostics",
      `Error detecting project: ${err.message}`,
      err,
    );
  }
  ...
  const formatterConfig = vscode.workspace.getConfiguration(
    "winccoa.astyleFormatter",
  );
  ...
  ExtensionOutputChannel.info(
    "Diagnostics",
    `   Astyle Formatter: ${formatterEnabled ? "✓ Enabled" : "✗ Disabled"}`,
  );
  if (formatterEnabled) {
    ExtensionOutputChannel.info(
      "Diagnostics",
      `      Format on Save: ${formatterOnSave ? "Yes" : "No"}`,
    );
  }

  ExtensionOutputChannel.info(
    "Diagnostics",
    `   Syntax Check: ${syntaxCheckEnabled ? "✓ Enabled" : "✗ Disabled"}`,
  );
  if (syntaxCheckEnabled) {
    ExtensionOutputChannel.info(
      "Diagnostics",
      `      Check on Save: ${syntaxCheckOnSave ? "Yes" : "No"}`,
    );
  }
  ...
  ExtensionOutputChannel.info(
    "Diagnostics",
    "   ✓ IntelliSense (983 built-in functions)",
  );
...
async function insertTemplate(
  ...
  if (
    ...
    vscode.window.showErrorMessage(
      "Template insertion only works in .ctl files",
    );
    return;
  }
  ...
  try {
    ...
    let origLibRelPathWithoutExtension = relPath.replace(
    ...
    if (templateType === "Test") {
      ...
      if (relPath.includes("scripts/tests/")) {
        ...
        origLibRelPathWithoutExtension = origLibRelPath.replace(
          /\.(ctl|ctrl|ctlpp|ctrlpp)$/,
          "",
        );
      } else {
    ...
    templateContent = templateContent
      ...
      .replace(
        /\$origLibRelPathWithoutExtension/g,
        origLibRelPathWithoutExtension,
      )
    ...
    ExtensionOutputChannel.info(
      "Template",
      `${templateType} template inserted: ${fileName}`,
    );
    vscode.window.showInformationMessage(
      `${templateType} template inserted successfully`,
    );
  } catch (error) {
    ExtensionOutputChannel.error(
      "Template",
      `Failed to insert template: ${error}`,
    );
...
async function setupCoreExtensionIntegration(context: vscode.ExtensionContext) {
  ...
  coreApi.onDidChangeProject((project: any) => {
    if (project) {
      ExtensionOutputChannel.info(
        "CoreIntegration",
        `Project changed: ${project.name} → Clearing path cache`,
      );
  ...
  if (currentProject) {
    ...
    ExtensionOutputChannel.debug(
      "CoreIntegration",
      `  Project path: ${currentProject.projectDir}`,
    );
    ExtensionOutputChannel.debug(
      "CoreIntegration",

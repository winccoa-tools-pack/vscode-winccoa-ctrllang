...
async function fetchProjectInfo(): Promise<ProjectInfo | null> {
    ...
    switch (globalSettings.pathSource) {
        case 'api':
            ...
            break;
        case 'workspace':
            result = await fetchFromWorkspace();
            break;
        case 'manual':
    ...
    // v0.4.0: Sync project info to symbol cache for dependency resolution
    symbolCache.setProjectInfo(result);
...
connection.onHover(async (params: TextDocumentPositionParams): Promise<Hover | null> => {
    ...
    // Ensure project info is loaded for dependency resolution
...
// v0.4.0: Use DefinitionService for all go-to-definition requests
connection.onDefinition(async (params: TextDocumentPositionParams): Promise<Definition | null> => {
    const doc = documents.get(params.textDocument.uri);
    if (!doc) return null;
    
    return definitionService.handle(doc, params.position);
});

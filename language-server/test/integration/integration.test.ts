...
import { resolveUsesPath } from '../../src/usesResolver';
import { pathToFileURL } from 'url';

suite('Language Server Integration Tests', () => {
    const testWorkspacePath = path.resolve(__dirname, '../../../test-workspace');
    const scriptsPath = path.join(testWorkspacePath, 'scripts');
    ...
    function simulateGoToDefinition(
        ...
        for (const dep of dependencies) {
            ...
            if (!fs.existsSync(resolvedPath)) {
                // Try parent directory's libs folder (for fixtures/ subdirectory)
                resolvedPath = path.join(basePath, '..', 'libs', `${dep}.ctl`);
            }
    ...
    test('finds Circle struct from DataStructures.ctl', () => {
        const testFile = path.join(fixturesPath, 'TestGoToDefinition.ctl');
        const testContent = fs.readFileSync(testFile, 'utf-8');
        
        const result = simulateGoToDefinition(testFile, 'Circle', testContent);
        
        assert.strictEqual(result.found, true, 'Circle should be found');
        assert.ok(result.filePath!.includes('DataStructures.ctl'));
        assert.strictEqual(result.line, 32, 'Circle definition should be at line 32');
    });
